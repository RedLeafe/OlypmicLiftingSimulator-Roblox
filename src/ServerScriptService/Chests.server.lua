...
local function CollectChest(player: Player, chest: Model)
    ...
    if area ~= "Daily" and area ~= "Group" then
    ...
    elseif currency == "Daily" then
        PlayerData.AdjustStrengthNoMult(player, reward)
        PlayerData.AdjustGems(player, (reward / 10))
    elseif currency == "Group" then
        if player:IsInGroup(3537205) then
           PlayerData.AdjustStrengthNoMult(player, (reward * 2))
           PlayerData.AdjustGems(player, ((reward * 2) / 10))
        end
    end
